...
ias.graph = (function (graph) {
    ...
    graph.Pin.prototype.filter = function () {
        var show = ias.filter.options.networks[this.cohort.getNetwork()];
        show = show && this.isSameStatus(ias.filter.options.enrollmentStatus);
...
ias.graph = (function (graph) {
    ...
    graph.map = function (svg) {
        ...
        function filterUpdate(event) {
            ...
            gpins_country.style("display", ias.filter.options.viewCountryPins ? 'inline' : 'none');
            gpins_cohort.style("display", ias.filter.options.viewCohortPins ? 'inline' : 'none');
        ...
        ias.filter.options.addListener([ias.filter.NETWORKS, ias.filter.YEAR, ias.filter.ENROLLMENT_STATUS], filterUpdate);
...
ias.graph = (function (graph) {
    ...
    graph.legend = function (svg) {
        ...
        //
        ias.filter.options.addListener(ias.filter.BACKGROUND_INFO, filterUpdate);
